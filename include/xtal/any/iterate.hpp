...
template <typename ...Ts> concept   countee_q  = iota_q<Ts...>;//unfalse_p<countee<Ts>::value...>;
...
template <typename T>
using     spanned_t = _v3::ranges::subrange<iterator_t<T>, sentinel_t<T>>;
XTAL_LET  spanned_f = [] (XTAL_DEF x)
XTAL_0FN_(spanned_t<decltype(x)>(XTAL_REF_(x)));

...
template <class T >
//\
struct   pack_item<T> : _xtd::decay_value_reference<T> {using supertype = T;};
struct   pack_item<T> {using supertype = T; using type = T;};
/*/
template <atomic_q T > struct   pack_item<T> : _xtd::decay_value_reference<T> {using supertype = T;};
template <class T                >	struct   pack_item<T> : _xtd::    decay_value<T> {using supertype = T;};
template <_detail::dismember_q T >	struct   pack_item<T> : _xtd::decay_value_reference<T> {using supertype = T;};
/***/
...
template <template <class ...> class F=pack_t>
XTAL_DEF_(return,inline,let)
prepack_f(pack_size_q auto &&...ts)
noexcept -> decltype(auto)
{
	return [&] (auto &&...us) XTAL_0FN_(to) (repack_f(XTAL_REF_(ts)..., XTAL_REF_(us)...));
};
...
template <class ...Ts> concept idiopack_q = pack_q     <Ts...> and (...and same_q<Ts, repack_t<Ts>>);

...
noexcept -> decltype(auto)
{
	...
	XTAL_IF0
	XTAL_0IF (xtd::rvalue<T_ref &&>) {return XTAL_RET_(get<K>(destruct_f(XTAL_REF_(t))));}
...
template <auto ...Is>
XTAL_VAL_(return,inline,let)
pack_item_f(seek_in_t<Is...>, auto &&...ts)
noexcept -> decltype(auto)
{
...
concept  pack_list_q = 0 == pack_size_v<T> or
	[] <auto ...Is> (seek_in_t<Is...>) XTAL_0FN_(to) (...and pack_item_q<T, Is>) (seek_to_t<pack_size_v<T>> {});
...
*/
XTAL_VAL_(let) pack_f = []<class ...Us> (Us &&...us)
...
struct   repack
{
	...
	make(seek_in_t<Is...>) -> pack_t<interpack_item_t<Is, Ts...>...>;

	using type = decltype(make(seek_to_t<pack_size_v<Ts...>>()));
...
noexcept -> decltype(auto)
{
	return [&]<auto ...Is> (bond::seek_in_t<Is...>)
		XTAL_0FN_(to) (F<interpack_item_t<Is, decltype(ts)...>...>{pack_item_f<Is>(ts...)...})
	(bond::seek_to_t<pack_size_v<decltype(ts)...>>{});
};

	...
	public:
		...
		struct series
		{
			...
			public:
				...
				{
					auto &s = get();
					auto constexpr x = realized::template patio_y<-1> (N);
				...
				requires bit_ceiling_q<N, 1>
				{
					...
					if constexpr (N_rotate&1)
					{
						that.transmute([] (U u) XTAL_0FN_(_std::conj(u*realized::template ratio_y<1>(N))));
					}
					return that;
				}

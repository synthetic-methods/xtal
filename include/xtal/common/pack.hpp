...
template <typename T          > concept  pack_part_ps = [] <size_t ...I>
	(seek_t<I...>) XTAL_0FN_(every_q<pack_part_p<T, I>...>)
...
struct pack
{
	...
	using    size   = _detail::pack_size  <type>;
	using    size_t = _detail::pack_size_t<type>;
	XTAL_LET size_v = _detail::pack_size_v<type>;
	
	template <size_t N> using part   = _detail::pack_part  <type, N>;
	template <size_t N> using part_t = _detail::pack_part_t<type, N>;
...
template <typename    T > concept pack_p = _detail::pack_size_p<T> and _detail::pack_part_ps<T>;

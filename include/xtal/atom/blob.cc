...
TAG_("blob")
{
	...
	TRY_("blob: allocation stepping") {
		size_type i{};
		TRUE_(_detail::aligned<std::byte   >::advance(i) == 0x0);
		TRUE_(_detail::aligned<std::int16_t>::advance(i) == 0x2);
		TRUE_(_detail::aligned<std::byte   >::advance(i) == 0x4);
		TRUE_(_detail::aligned<std::byte   >::advance(i) == 0x5);
		TRUE_(_detail::aligned<std::int16_t>::advance(i) == 0x6);
		TRUE_(_detail::aligned<std::byte   >::advance(i) == 0x8);
		TRUE_(_detail::aligned<std::int16_t>::advance(i) == 0xA);

	//	TRUE_(0x10 == blob_t<std::byte, std::int16_t, std::byte, std::byte, std::int16_t, std::byte, std::int16_t>::size());
	}
}
/***/

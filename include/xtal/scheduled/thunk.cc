...
TAG_("thunk", "process")
{
	...
	//	using namespace scheduled;
		...
		using V_gate = message::inferred_t<class gate_a, typename atom::realized::alpha_t>;
...
//	using namespace scheduled;
	...
	using alpha_t = typename atom::realized::alpha_t;
	...
	fx_gate >>= U_scope(N_store)*0; _v3::ranges::copy(fx_gate, u_store.begin());
	TRUE_(u_store == U_store {  7,  1,  1, -1,  1, -1, -1, 77});

	fx_gate <<= (U_cue) 4 << (V_gate) 11;
	
	fx_gate >>= U_scope(N_store)*1; _v3::ranges::copy(fx_gate, u_store.begin());

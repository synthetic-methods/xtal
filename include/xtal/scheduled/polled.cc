...
TAG_("polled", "process")
{
	...
	//	using namespace scheduled;
		...
		using U_polled = polled_t<spooled<N_spool>>;
		using U_scope = message::scope_t<>;
		using U_cue = cell::cue_s<>;
		
		using V_gate = message::label_t<typename bond::realized::alpha_t, struct __gate__>;
		using U_gate = process::confined_t<typename U_polled::template inqueue<V_gate>>;
...
//	using namespace scheduled;
	...
	using U_scope  = message::scope_t<>;
	using U_store  = _std::array<typename bond::realized::alpha_t, N_store>;
	U_store u_store {};
...
TAG_("polled", "processor")
{
	using namespace processor;
	TRY_("drive material") {polled_processor<resourced::stored<>>();}
//	TRY_("drive virtual")  {polled_processor<>();}// TODO?

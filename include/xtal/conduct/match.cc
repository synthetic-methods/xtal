...
TAG_("match")
{
	TRY_("construct")
	{
		...
		using target_t = match_s<source_t>;
		using target_u = match_s<>;
	...
	auto constexpr maybe = [] (match_s<> g)
	XTAL_0FN {
		if (g) {
			return true;
		}
	...
	TRY_("condition")
	{
		  TRUE_(maybe(match_s<>(1)));
		UNTRUE_(maybe(match_s<>(0)));
		UNTRUE_(maybe(match_s<>( )));

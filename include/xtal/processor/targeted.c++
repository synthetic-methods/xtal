...
{
	using buffer   =  common::buffer<(1<<5)>;
	using buffed   = typename buffer::type;
	using vector_t = typename buffed::vector_t<alpha_t>;
	using  visor_t = typename buffed:: visor_t<alpha_t>;
	using respan_t = message::respan_t<visor_t>;
...
//	xhs <<= common::bundle_f(  serial_m, respan_m);                                                                 // initialize via influx?
	xhs >>= common::bundle_f(  serial_m, respan_m); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{00, 11, 22}));// initialize via efflux!
	xhs >>= common::bundle_f(++serial_m, respan_m); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{33, 44, 55}));// advance then efflux...
	xhs >>= common::bundle_f(++serial_m, respan_m); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{66, 77, 88}));// advance then efflux...
	xhs <<= bias_t((alpha_t) (11 + 1));
	xhs >>= common::bundle_f(++serial_m, respan_m); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{111, 122, 133}));// advance then efflux...
...
void respan_internal_interrupt__test()
{
	...
	xhs <<= common::bundle_f(0, (bias_t) (alpha_t) (00));
	xhs <<= common::bundle_f(1, (bias_t) (alpha_t) (11));
	xhs <<= common::bundle_f(2, (bias_t) (alpha_t) (22));
	xhs >>=  seq++;
	REQUIRE(4 == xhs.size());
	REQUIRE(_v3::ranges::equal(xhs, _std::vector{00, 22, 44, 55}));

	xhs <<= common::bundle_f(2, (bias_t) (alpha_t) (00));
	xhs >>=  seq++;
	REQUIRE(4 == xhs.size());

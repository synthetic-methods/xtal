...
void test__respan_provision()
{
	...
	auto xhs = monomer_t<add_t, provide>::bind_f(lhs, rhs);
	...
	xhs >>= bundle_f(respan_m, sequel_m++); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{00, 11, 22}));// initialize via efflux!
	xhs >>= bundle_f(respan_m, sequel_m++); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{33, 44, 55}));// advance then efflux...
	xhs >>= bundle_f(respan_m, sequel_m++); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{66, 77, 88}));// advance then efflux...
	xhs <<= bias_t((alpha_t) (11 + 1));
	xhs >>= bundle_f(respan_m, sequel_m++); REQUIRE(_v3::ranges::equal(vector_m, _std::vector{111, 122, 133}));// advance then efflux...

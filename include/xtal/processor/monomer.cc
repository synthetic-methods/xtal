...
void monomer_provision__advancing()
{
	...
	auto xhs = U_mixer::binds_f(lhs, rhs);
...
void monomer_provision__provisioning()
{
	...
	auto xhs = monomer_t<U_add, provide>::binds_f(lhs, rhs);
...
void monomer_chaining__rvalue()
{
	...
	auto yhs = mul_op::binds_f(mix_op::binds_f(let_f(_01), let_f(_10)));
...
void monomer_chaining__lvalue()
{
	...
	auto  lhs = let_f(_01); TRUE_(&lhs.head() == &processor::let_f(lhs).head());
	auto  rhs = let_f(_10); TRUE_(&rhs.head() == &processor::let_f(rhs).head());
	auto  xhs = mix_op::binds_f(lhs, rhs);
	auto  yhs = mul_op::binds_f(xhs);
...
void monomer_chaining__shared()
{
	...
	auto _xx = idx_fn::binds_f();
	auto xhs = mix_op::binds_f(_xx);
	auto lhs = mix_fn::binds_f(xhs, let_f(_01));
	auto rhs = mix_fn::binds_f(xhs, let_f(_10));
	auto yhs = mix_fn::binds_f(lhs, rhs);

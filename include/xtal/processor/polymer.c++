...
TEST_CASE("xtal/processor/polymer.hpp: true")
{
	...
	using fate_t = processor::monomer_t<gate_t, collect<>, typename stage_t::gauge>;
...
//	auto  vox_o = vox_t::bond_f(fate_o);
	...
	vox_o <<= bundle_f(event_t(62, 0), bias_t(1)); REQUIRE(1 == vox_o.spool().size());
	vox_o <<= bundle_f(event_t(65, 0), bias_t(1)); REQUIRE(2 == vox_o.spool().size());
	vox_o <<= bundle_f(event_t(69, 0), bias_t(1)); REQUIRE(3 == vox_o.spool().size());
	vox_o <<= bundle_f(event_t(69, 0), bias_t(1)); REQUIRE(4 == vox_o.spool().size());
	vox_o >>= sequel_u(N_size)*0;
